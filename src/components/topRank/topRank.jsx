...
import { getTopRankDataThunkAction } from './logic/reducer';
...
export default function TopRank() {
  ...
  const renderItem = (item, index) => {
    ...
    return <ListItem alignItems="flex-start" className="topRank__item" key={`${item.CreateDate}_${item.UserName}`}>
      ...
      <div className='topRank__item--st'>
        {
          isShowIcon && 
          <AutoSizeImage isResize={false} src={srcImage} width={88} height={86} />
        }
      </div>
    </ListItem>
  }
  return (
    <React.StrictMode>
      ...
      <div className={`btn__topbxh ${isShowRank ? 'active' : ''}`} onClick={(e) => handleShowTopRank(e)}>
